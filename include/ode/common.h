...
#define dSqrt(x) (FastSqrt(x)) //((float)sqrtf(float(x)))			/* square root */
#define dRecipSqrt(x) ((float)(1.0f/dSqrt(float(x))))		/* reciprocal square root */
#define dSin(x) FastSin(x) //((float)sinf(float(x)))				/* sine */
#define dCos(x) FastCos(x)// ((float)cosf(float(x)))				/* cosine */
#define dFabs(x) FastAbs(x) //((float)fabsf(float(x)))			/* absolute value */
#define dAtan2(y,x) FastAtan2(y,x) // ((float)atan2f(float(y),float(x)))		/* arc tangent with 2 args */
#define dFMod(a,b) ((float)fmodf(float(a),float(b)))		/* modulo */
#define dCopySign(a,b) ((float)copysignf(float(a),float(b)))
#define dAsin(x) FastAsin(x)
#define dAcos(x) FastAcos(x)
